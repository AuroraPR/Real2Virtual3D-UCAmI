...
public class control : MonoBehaviour
{
    ...
    {
        string[] files = System.IO.Directory.GetFiles("Assets/javi", "*.json");
        ...
        foreach (string file in files)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                float x = float.Parse(coordinates[0]) * 10f;
                float y = (1-float.Parse(coordinates[1])) * 10f;
                float z = float.Parse(coordinates[2]) * 10f;
